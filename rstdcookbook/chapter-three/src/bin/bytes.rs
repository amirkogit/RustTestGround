...
fn main() {
    println!("Handling bytes");

    let binary_nums = vec![2,3,12,8,5,0];

    // wrap a binary collection in a cursor to provide seek functionality
    ...
    // reading advances the internal position, so now we read the second
    ...
    // ------------------------------------------
    // read and write in specific endianess
    buff.set_position(0);
    let as_u32 = buff.read_u32::<LittleEndian>().expect("Failed to read bytes");
    println!("First four bytes as u32 in little endian order:\t{}", as_u32);
    ...
    buff.set_position(0);
    buff.read_u16_into::<LittleEndian>(&mut read_buffer).expect("Failed to read all bytes");
    println!("All bytes as u16s in little endian order: {:?}", read_buffer);
}